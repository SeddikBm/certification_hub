...
def rewrite_node(state: GraphState) -> dict:
    ...
    try:
        rewritten = QueryRewriter().rewrite(message)
    except RagChatError as exc:
        # Rewriting is an optimisation, not a hard dependency — degrade to
        # the original message rather than failing the whole request.
